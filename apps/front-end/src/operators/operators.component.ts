...
export class OperatorsComponent implements OnInit {
  ...
  remove(id: number, voidDto: VoidDto) {
    return this.operatorsService.delete(id, voidDto).pipe(
      tap(() => {
        this.snackBar.open(`Operator deleted successfully`, 'success');
      }),
      catchError((error) => {
        this.snackBar.open(
          `An error occurred while deleting the operator: ${error.statusText}`,
          'error'
        );
        throw error;
      })
